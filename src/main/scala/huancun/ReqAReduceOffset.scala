...
//  io.resp_out <> io.resp_in
  ...
  when(beat_offset =/= 0.U){
    ...
    val mask_parse_by_size = WireInit(0.U(beatBytes.W))
    dontTouch(beat_valids_0)
    dontTouch(beat_valids_1)
    dontTouch(beat_valid)
    dontTouch(mask_data_vec)
    dontTouch(mask_data)
    dontTouch(mask_parse_by_size)

    // ---------- Parse data -------------//
    val temp = mask_all_1.asUInt >> (beatBytes.U - in_a.size)
    mask_parse_by_size := in_a.mask | temp.asUInt
    beat_valids_0.zipWithIndex.foreach {
      case (valid, i) =>
        valid := Mux(i.U < (beatBytes.U - beat_offset), mask_parse_by_size.asBools(i), false.B)
    }
