...
class MSHR()(implicit p: Parameters) extends BaseMSHR[DirResult, SelfDirWrite, SelfTagWrite] with HasClientInfo {
  ...
  io.tasks.source_c.valid := io.enable && (Mux(!req.fromCmoHelper, !s_release && s_writeprobe && !will_schedule_writeprobe && w_sinkcack, !s_release && w_probeack && s_writeprobe && w_sinkcack && !will_schedule_writeprobe) || !s_probeack && s_writerelease && w_sinkcack && w_probeack)
  io.tasks.source_d.valid := io.enable && !s_execute && can_start && w_grantlast && s_writeprobe && w_sinkcack && w_probeacklast && s_transferput && !(waitRelease && !(nested_c_hit || nested_c_miss)) // TODO: is there dependency between s_writeprobe and w_probeack?
