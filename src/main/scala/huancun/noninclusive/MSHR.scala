...
class MSHR()(implicit p: Parameters) extends BaseMSHR[DirResult, SelfDirWrite, SelfTagWrite] with HasClientInfo {
  ...
  io.tasks.source_b.valid := io.enable && Mux(!req.fromCmoHelper, !s_probe && s_release, io.enable && !s_probe)
  io.tasks.source_c.valid := io.enable && (!s_release && w_probeack || !s_probeack && s_writerelease && w_sinkcack && w_probeack)
